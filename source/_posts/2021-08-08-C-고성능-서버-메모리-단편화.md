...
### Compression
`SOH`의 메모리는 객체가 얼마나 오래 살았느냐에 따라 0세대부터 2세대까지 세대를 구분한다. GC가 한 번 실행될 때 사용이 끝난 메모리는 해제되고, 아직 사용중인 메모리는 다음 세대로 승격한다. 이 때 살아남은 메모리들은 `압축(Compression)`의 과정을 거친다. 압축이란 메모리 단편화를 줄이기 위해, 살아남은 메모리들을 사이사이 공백이 없도록 한 공간으로 몰아서 재배치하는 동작을 말한다. 실제로 관리 힙 내부에서 객체들은 세대별로 모아두어야 하기 이문에, 메모리 해제 및 승격을 거친 후에는 세대별 구획에 맞춰 메모리를 재정렬하는 과정이 반드시 필요하다.
{% asset_img 01.png %}
...
조금 더 신경을 쓴다면 서비스 도중 메모리 청크를 추가할당 할 때의 처리부분 정도가 있겠다. Pool에 남아있는 청크이 개수가 좀 모자란다 싶을 때는 CAS 연산으로 소유권을 선점한 스레드가 추가메모리를 할당해주면 lock없이도 메모리를 늘릴 수 있다. 해당 구현체의 멤버변수들만 붙여보면 아래와 같다.
...
우리가 겪었던 메모리 단편화 가장 주된 요인은 네트워크 IO용 바이트 버퍼의 pinning 때문이었다. 어느정도 수준까지는 별 문제 없는데 부하를 세게 걸면 점유 메모리가 계속 증가하고 가라않질 않았다. 이건 C++도 마찬가지지만 외형적으로만 관측하면 메모리 누수처럼 보이기 때문에, 단편화가 원인일 것이라는 의심을 하기까지도 많은 검증의 시간이 필요했다.
