...
모든 tls 변수에 동일한 값의 복제본을 저장해 두려는 경우가 있다. 예를들어 스레드가 3개 있으면, 메모리 공간상에 각 스레드를 위한 변수 3개가 있고, 이들 모두에 같은 의미를 가지는 인스턴스를 각각 생성해 할당하는 경우를 말한다. **서로 다른 스레드끼리 공유해야 할 자원이 있을 때, 해당 자원에 lock이 없이 접근하고 싶다면** tls를 이용해 각 스레드마다 자원을 따로 만들어 각자 자기 리소스를 쓰게 하면 된다.
...
현재 프로젝트에서 구현한 네트워크 모듈 및 스레드 모듈은 거의 다 VC++로 만들었던 이전 프로젝트의 기반을 .net managed api를 이용해 재구현한 수준에 가깝다. 내가 기존부터 익숙하게 해오던 설계 및 최적화 방식 등을 따르고자(=C++에서 짜던거랑 똑같이 만들고자) AsyncLocal의 복사를 막아주었지만 이게 유일한 정답은 아닐것이다. C#은 내가 잘 모르는 나름의 방법으로 비동기 네트워크 컨트롤을 더 효율적으로 하는 방법이 있을 수도 있으니, 여러 구현 예시중의 하나로만 참고 바란다. 
